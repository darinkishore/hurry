...
/// workaround allows us to use COURIER_DATABASE_URL in .env instead, avoiding
/// conflicts with other packages (like hurry) that also need databases.
///
...
fn sqlx_env_var_tests() {
    println!("cargo:rerun-if-env-changed=COURIER_DATABASE_URL");
    if let Ok(url) = std::env::var("COURIER_DATABASE_URL") {
