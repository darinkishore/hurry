...
use crate::cmd::cross::build::Options;
...
pub async fn exec(arguments: Vec<String>) -> Result<()> {
    ...
    match command.as_str() {
        "build" => {
            let cmd = CommandOptions::<Options>::parse(&arguments)?;
            if cmd.opts.help {
                ...
                let mut cmd = CommandOptions::<Options>::command();
                cmd = cmd.about("Run `cross build` with Hurry build acceleration");
                cmd.print_help()?;
                return Ok(());
            }
            build::exec(cmd.into_inner()).await
        }
